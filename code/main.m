...
%% Analytical solution
tic
d1 = (log(S0/K)+muRN*T)/(sigma*sqrt(T));
% d2 = d1 - sigma*sqrt(T);
...
d1 = (log(Sa/K)+muRN*(T-t))./(sigma*sqrt(T-t));
d2 = (log(Sa/K)+muRN*(T-t))./(sigma*sqrt(T-t));
...
d1 = (xa-k+muRN*(T-t))./(sigma*sqrt(T-t));
d2 = (xa-k+muRN*(T-t))./(sigma*sqrt(T-t));
